...
export default function Nastavenia() {
  ...
  const testBridge = async () => {
    ...
    try {
      const r = await fetch(`${form.bridgeUrl}/api/health`, { signal: AbortSignal.timeout(3000) });
    ...
    <div className="p-4 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        ...
        <div className="mt-4 bg-slate-50 rounded-xl p-4">
          ...
          <p className="text-xs text-slate-500 mt-2">Lokálny HTTPS server · Self-signed certifikát · Token autentifikácia (hlavička X-Bridge-Token)</p>
        </div>
      </div>
    </div>
  );
